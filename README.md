...
> **Verified in CI — and honest about its limits.** These invariants are covered by an automated Jest suite that runs in GitHub Actions on every push and pull request (`.github/workflows/jest.yml`) — **492 tests** in total, including a dedicated **21-test** regression file (`aartiq-browser/tests/approval-ticket-security.test.js`) for the approval-ticket and capability-controller system. Authorization decisions are emitted as pure, inspectable `AuthorizationDecision` objects that the executor consumes without re-deriving approval.
>
