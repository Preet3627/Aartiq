...
function SourceCard({ source, index }: { source: NonNullable<ResearchReportProps['sources']>[0]; index: number }) {
  const domain = (() => { try { return new URL(source.url).hostname.replace('www.', ''); } catch { return source.name; } })();
  return (
    <motion.div
    ...
    >
      <div className="w-6 h-6 rounded bg-white/8 flex items-center justify-center overflow-hidden shrink-0">
        {source.favicon ? (
          ...
          <span className="text-[10px] text-zinc-400">{source.name.charAt(0)}</span>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        <h1 className="text-lg font-bold text-zinc-100 leading-tight">{title}</h1>
      ...
      {executiveSummary && (
        ...
        >
          ...
          <p className="text-xs text-zinc-300 leading-relaxed">{executiveSummary}</p>
        </motion.div>
      )}
            ...
            {showTOC && (
              ...
              >
                <div className="px-2 pb-2 space-y-0.5">
                  {sections.map(s => (
                    <TOCItem key={s.id} section={s} onClick={() => { setShowTOC(false); }} />
                  ))}
                </div>
              </motion.div>
            )}
          ...
          <div className="px-4 py-3 space-y-3">
            {contradictions.map((c, i) => (
              <div key={i} className="space-y-1">
                <div className="text-[10px] font-medium text-amber-400/80 uppercase tracking-wider">{c.type.replace(/_/g, ' ')}</div>
                ...
                {c.ratio && <div className="text-[10px] text-amber-400/60">Discrepancy: {c.ratio}x</div>}
              </div>
            ))}
          </div>
        </motion.div>
