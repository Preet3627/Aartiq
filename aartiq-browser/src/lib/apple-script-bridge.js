...
async function createVoiceCommandShortcut(commandName, params = {}) {
  const shortcutScript = `
    on run
      ...
      tell application "Aartiq"
        activate
        -- Send command to Aartiq via URL scheme
        open location "aartiq://chat?message=" & userInput
      end tell
      
      display notification "Sent to Aartiq" with title "Aartiq"
    end run
  `;
...
async function createDictationShortcut() {
  const script = `
    on run
      ...
      tell application "System Events"
        tell application process "Aartiq"
      ...
      if voiceInput is not "" then
        tell application "Aartiq"
          activate
          open location "aartiq://chat?message=" & voiceInput
        end tell
        
        display notification "Sent: " & voiceInput with title "Aartiq"
      end if
    end run
  `;
...
function generateDpeechRecognitionScript() {
  const script = `
    -- macOS Speech Recognition for Aartiq
...
function generateVoiceChatScript() {
  const script = `
    -- Voice Chat workflow for Aartiq
    ...
    on run
      ...
      if length of dictatedText > 0 then
        -- Step 4: Send to Aartiq
        tell application "Aartiq"
          activate
          open location "aartiq://chat?message=" & dictatedText & "&speak=true"
        end tell
      else
...
function generateScheduleWithReminderScript() {
  const script = `
    -- Schedule task with reminder in Aartiq
    on run
      ...
      tell application "Aartiq"
        activate
        open location "aartiq://schedule?task=" & taskName & "&cron=" & scheduleTime
      end tell
      
      display notification "Scheduled: " & taskName with title "Aartiq"
    end run
  `;
