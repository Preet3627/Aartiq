...
const INPUT_TYPE_MAP: Record<string, ProfileKey> = { email: 'email', tel: 'phone' };
...
const LABEL_MAP: Array<[string[], ProfileKey]> = [
...
function matchAutocomplete(f: FieldDescriptor): { profileKey: ProfileKey; confidence: number; matchedBy: string } | null {
...
function matchInputType(f: FieldDescriptor): { profileKey: ProfileKey; confidence: number; matchedBy: string } | null {
  const key = INPUT_TYPE_MAP[(f.type || 'text').toLowerCase()];
  return key ? { profileKey: key, confidence: 90, matchedBy: 'inputType' } : null;
}

function matchName(f: FieldDescriptor): { profileKey: ProfileKey; confidence: number; matchedBy: string } | null {
  const name = normalize(f.name || '');
  if (!name) return null;
  if (NAME_MAP[name]) return { profileKey: NAME_MAP[name], confidence: 80, matchedBy: 'name' };
  for (const [pattern, pk] of Object.entries(NAME_MAP)) {
    if (name.includes(pattern)) return { profileKey: pk, confidence: 70, matchedBy: 'name' };
  }
  return null;
}

function matchLabel(f: FieldDescriptor): { profileKey: ProfileKey; confidence: number; matchedBy: string } | null {
  const label = normalize(f.label || '');
  if (!label) return null;
  for (const [patterns, pk] of LABEL_MAP) {
    if (patterns.some((p) => label === p)) return { profileKey: pk, confidence: 75, matchedBy: 'label' };
    if (patterns.some((p) => label.includes(p))) return { profileKey: pk, confidence: 65, matchedBy: 'label' };
  }
  return null;
}

function matchPlaceholder(f: FieldDescriptor): { profileKey: ProfileKey; confidence: number; matchedBy: string } | null {
  const ph = normalize(f.placeholder || '');
  if (!ph) return null;
  for (const [patterns, pk] of LABEL_MAP) {
    if (patterns.some((p) => ph === p || ph.includes(p))) return { profileKey: pk, confidence: 50, matchedBy: 'placeholder' };
  }
  return null;
}

export function matchField(f: FieldDescriptor): { profileKey: ProfileKey; confidence: number; matchedBy: string } | null {
...
export function matchFields(fields: FieldDescriptor[], profile: Profile): FieldMatch[] {
  const assignments = new Map<ProfileKey, FieldMatch>();
  ...
  for (const f of fields) {
    ...
    if (usedIndices.has(f.index)) continue;
