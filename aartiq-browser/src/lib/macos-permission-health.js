...
async function getMacOSPermissionHealth({ promptAccessibility = false } = {}) {
  ...
  if (screenStatus !== 'granted') {
    ...
    issues.push({
      ...
      permission: 'screen',
      message: 'macOS reports Screen Recording as granted, but capture still fails. Relaunch Aartiq; if it persists, reset the stale Screen Recording grant.',
    });
  }
  ...
  if (!accessibilityTrusted) {
    issues.push({
      ...
      message: 'Accessibility is not trusted for the current Aartiq process.',
    });
  }
  ...
  if (!bundleIdentifier) {
    issues.push({
      ...
      permission: 'identity',
      message: 'Aartiq is running without a packaged .app bundle identity, so macOS may grant permissions to Electron, Terminal, or the launcher instead.',
    });
  }
...
async function resetMacOSPermissions(types = ['screen', 'accessibility']) {
  ...
  if (!bundleIdentifier) {
    return {
      success: false,
      error: 'No packaged app bundle identifier found. Launch the built Aartiq.app, not Electron through a terminal, then retry.',
    };
  }
  ...
  return {
    ...
    bundleIdentifier,
    reset,
    message: 'Permissions were reset for this app identity. Quit and reopen Aartiq, then grant the prompts again.',
  };
}
