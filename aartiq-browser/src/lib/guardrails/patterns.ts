...
const CRITICAL: ThreatPattern[] = [
  ...
  {
    type: ThreatType.DANGEROUS_COMMAND,
...
const HIGH: ThreatPattern[] = [
  ...
  {
    type: ThreatType.ENCODED_PAYLOAD,
    pattern: /(?:\\x[0-9a-f]{2}|\\u[0-9a-f]{4}|%[0-9a-f]{2}[0-9a-f]{2}){4,}/i,
    severity: 'high',
    modes: ['normal', 'strict'],
  },
  {
    type: ThreatType.SUSPICIOUS_URL,
...
const MEDIUM: ThreatPattern[] = [
  ...
  {
    type: ThreatType.CROSS_SITE_SCRIPT,
...
const LOW: ThreatPattern[] = [
  {
    type: ThreatType.ZERO_WIDTH_CHAR,
    pattern: /[\u200B-\u200F\u2028-\u202F\uFEFF]/,
    severity: 'low',
    modes: ['strict'],
  },
...
export const ALL_PATTERNS: ThreatPattern[] = [...CRITICAL, ...HIGH, ...MEDIUM, ...LOW];

export function getPatternsForMode(mode: 'normal' | 'strict'): ThreatPattern[] {
  return ALL_PATTERNS.filter(p => p.modes.includes(mode));
}
