...
function getAvailableModels() {
    try {
        ...
        if (process.platform === 'darwin') {
            ...
            appDataDir = path.join(process.env.APPDATA, 'aartiq');
        } else {
...
async function selectModel() {
    ...
    if (models.length === 0) {
        console.log('\nNo models configured in Aartiq browser.');
        console.log('Please configure AI models in Aartiq Settings first.\n');
...
function printHelp() {
    console.log(`
🚀 Aartiq CLI v${getVersion()}

Usage:
  aartiq ask "<prompt>"              Send a prompt to the AI
  aartiq search "<query>"            Search the web
...
Examples:
  aartiq ask "How do I create a PDF?"
  aartiq ask "Summarize news" -m openai
  aartiq select-model
  aartiq chat
  aartiq models
    `);
...
async function handleAsk() {
    if (!payload) {
        ...
        console.log('Usage: aartiq ask "<prompt>"');
        process.exit(1);
    }
...
async function main() {
    ...
    if (flags.version) {
        console.log(`Aartiq CLI v${getVersion()}`);
        process.exit(0);
    }
    ...
    switch (command) {
        ...
        case 'models':
            ...
            if (models.length === 0) {
                ...
                console.log('\nConfigure models in Aartiq browser settings.');
            } else {
                console.log('\n🤖 Available Models:\n');
                models.forEach((m, i) => {
                    const selected = m.id === cliConfig.lastModel ? '👉 ' : '  ';
